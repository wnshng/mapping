...
function getBookmarkletCode() {
  ...
  return `javascript:(()=>{const out=[];const seen=new Set();const links=[...new Set(Array.from(document.querySelectorAll('a[href*="/p/entry/place/"],a[href*="/entry/place/"],a[href*="/place/"]')))];links.forEach((a)=>{const href=new URL(a.getAttribute('href')||a.href||'',location.href).href;if(/^https?:\\/\\/map\\.naver\\.com\\/?([?#].*)?$/i.test(href))return;const container=a.closest('li,article,[role="listitem"],div');const raw=(container?container.innerText:a.innerText)||'';const lines=raw.split(/\\n+/).map((s)=>s.trim()).filter(Boolean);const name=((a.getAttribute('aria-label')||a.innerText||lines[0]||'').split('\\n')[0]||'').trim();if(!name||name.length<2||['지도','홈','MY','내장소','내장소보기'].includes(name.replace(/\\s+/g,'')))return;const address=lines.find((line)=>/([가-힣]+(시|도)\\s+)?[가-힣0-9]+(시|군|구)\\s+[가-힣0-9]+(로|길|동|읍|면)/.test(line))||lines.find((line)=>(/(로|길|동|리)/.test(line)&&/\\d/.test(line)))||'';const key=name+'|'+address+'|'+href;if(!seen.has(key)){seen.add(key);out.push([name,address,'','',''+href].join('|'));}});const text=out.join('\\n');if(!text){alert('장소를 찾지 못했습니다. 저장목록 리스트를 화면에 보이게 스크롤한 뒤 다시 실행하세요.');return;}if(navigator.clipboard&&window.isSecureContext){navigator.clipboard.writeText(text).then(()=>alert('복사 완료: 앱의 텍스트 가져오기에 붙여넣으세요.')).catch(()=>prompt('아래 텍스트를 복사하세요',text));}else{prompt('아래 텍스트를 복사하세요',text);}})();`;
}
